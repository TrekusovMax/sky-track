...
export const createSplitGreatCircle = (
	...
	const offsetPoint: [number, number] = [
		snappedCoord[0] * (1 - BACK_SHIFT_RATIO) +
			prevCoord[0] * BACK_SHIFT_RATIO,
		snappedCoord[1] * (1 - BACK_SHIFT_RATIO) +
			prevCoord[1] * BACK_SHIFT_RATIO
	]

	return {
		solidFeature: lineString(
			coords.slice(0, index + 1) as [number, number][]
		),
